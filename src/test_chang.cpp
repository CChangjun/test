...
bool calcOdometry(void)
{
    double wheel_l, wheel_r; // 바퀴의 회전량 [tick]
    double wheel_l_distance, wheel_r_distance; // 바퀴가 이동한 거리 [m]
    double delta_s, delta_theta; // 로봇의 전진 거리와 방향 변화
    static double last_theta = 0.0; // 이전 방향 저장
    ...
    last_theta = odom_pose[2]; // 현재 방향을 저장
